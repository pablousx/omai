...
Relai targets Linux global configuration and Omarchy's Quattro plugin interface. Keep provider-specific semantics explicit and preserve fields outside supported bindings. Changes to source or state formats require compatibility fixtures and documented migration behavior.
...
Tests must use temporary homes, fixture service managers, and local bare remotes. Do not point automated tests at live provider configuration, credentials, or the user's real Relai service. New failure-handling behavior needs tests for preserving existing data and recovering after interruption.
...
Before public distribution, copy the manifest, `qml/`, `scripts/`, `cmd/`, `internal/`, `go.mod`, `go.sum`, `mise.toml`, and license into an Omarchy user plugin folder named `pablousx.relai`. Stage the complete directory outside `~/.config/omarchy/plugins/` before moving it into place: copying individual source files into a watched plugin causes repeated shell reloads. Enable it with `omarchy plugin enable pablousx.relai --section right` after discovery. Normal setup then happens through **Set up Relai** in the panel. If a release is unavailable, choose **Build local copy and retry** there.

For compiler or headless development, the lower-level commands remain available:

```sh
./scripts/setup --source --install-only
~/.local/share/relai/bin/relai setup --yes --no-service \
  --remote /absolute/path/to/remote.git --providers codex,claude,opencode
~/.local/share/relai/bin/relai daemon run
```
