Relai 1.0 synchronizes supported global Codex, Claude Code, and OpenCode configuration from one editable source, with offline operation, ordinary Git transport, explicit conflicts, and recovery journals.
...
Install the plugin from https://github.com/pablousx/relai.git and choose **Set up Relai**. Complete the form and start sync inside the plugin window; it installs the companion executable automatically. Existing users should update the plugin checkout and choose **Update Relai**, which also runs in the panel. Unpublished local copies can explicitly build from source using the panel's retry action.
